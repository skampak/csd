...
int main(int argc, char** argv)
{
	...
	while ( fgets(buff, BUFFER_SIZE, fin) ) {
		...
		case 'C':
		{
			int sid[3];
			int distance;
			sscanf(buff, "%c %d %d %d %d", &event, sid, &distance, sid+1, sid+2);
			assert(sid[0] > 0);
			assert(sid[1] > 0);
			assert(sid[2] > 0);
			DPRINT("%c %d %d %d %d\n", event, sid[0], distance, sid[1], sid[2]);
		...
		/* Lookup an orphan planet
		 * K <oid> */
		case 'K':
		{
			int oid;
			sscanf(buff, "%c %d", &event, &oid);
			assert(oid > 0);
			DPRINT("%c %d\n", event, oid);
